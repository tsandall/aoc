...
func execute() (map[string]int, int) {
	registers := map[string]int{}
	var max int
	for _, instr := range input {
		v := registers[instr.cond.reg]
		var ok bool
		switch instr.cond.operator {
		case "==":
			ok = v == instr.cond.operand
		case "!=":
			ok = v != instr.cond.operand
		case "<":
			ok = v < instr.cond.operand
		...
		if ok {
			eval(registers, instr)
		}
		if v, ok := registers[instr.reg]; ok && v > max {
			max = v
		}
	}
...
func eval(registers map[string]int, instr instr) {
...
func parseInput(s string) (result []instr) {
...
func parseInstruction(s string) (result instr) {
	parts := strings.Fields(s)
	result.reg = parts[0]
	result.operator = parts[1]
	result.operand = parseInt(parts[2])
	result.cond.reg = parts[4]
	result.cond.operator = parts[5]
	result.cond.operand = parseInt(parts[6])
	return
}
...
var input = parseInput(`gug dec 188 if zpw >= 8
