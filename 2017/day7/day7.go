...
func part2() int {
	return unbalanced(input.Root())
}

func unbalanced(node string) int {

	// DFS to find unbalanced node.
	for edge := range input.edges[node] {
		if answer := unbalanced(edge); answer != 0 {
			return answer
		}
	}
	...
	for other := range input.edges[node] {
		w := weight(other)
		weights[other] = w
		if w1 == 0 {
			w1 = w
	...
	// Find bad node and calculate correct weight.
	for other, w := range weights {
		if w == badWeight {
			return input.nodes[other] + delta
		}
	}
...
func parseInput(s string) tree {
	result := tree{
		nodes: map[string]int{},
		edges: map[string]map[string]struct{}{},
	}
	for _, line := range strings.Split(s, "\n") {
		node, weight, edges := parseLine(line)
		result.edges[node] = edges
		result.nodes[node] = weight
	}
	return result
}

func parseLine(line string) (node string, weight int, edges map[string]struct{}) {
	parts := strings.Fields(line)
	node = parts[0]
	i, err := strconv.ParseInt(parts[1][1:len(parts[1])-1], 10, 64)
	if err != nil {
		panic(err)
	}
	weight = int(i)
	edges = map[string]struct{}{}
	for idx := 3; idx < len(parts); idx++ {
		edges[strings.Trim(parts[idx], ",")] = struct{}{}
	}
	return
}

var input = parseInput(`uwzmqi (57)
