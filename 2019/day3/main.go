...
func part2(a, b []step) {
	...
	for p, n := range aPoints {
		if m, ok := bPoints[p]; ok {
			if p.X > 0 && p.Y > 0 {
				if n+m < minSteps || minSteps == -1 {
					minSteps = n + m
				}
			}
		}
...
func getPoints(a []step) map[coord]int {

	var prev coord
	result := map[coord]int{}
	var n int
	...
	for _, s := range a {
		...
		switch s.Direction {
		case 'R':
			for i := 0; i <= s.Magnitude; i++ {
				pos := coord{X: prev.X + i, Y: prev.Y}
				if _, ok := result[pos]; !ok {
					result[pos] = n
				}
				n++
			}
			n-- // account for starting point
		case 'U':
			for i := 0; i <= s.Magnitude; i++ {
				pos := coord{X: prev.X, Y: prev.Y + i}
				if _, ok := result[pos]; !ok {
					result[pos] = n
				}
				n++
...
func move(p coord, s step) coord {
	switch s.Direction {
	case 'R':
		return coord{X: p.X + s.Magnitude, Y: p.Y}
	case 'L':
		return coord{X: p.X - s.Magnitude, Y: p.Y}
	case 'U':
		return coord{X: p.X, Y: p.Y + s.Magnitude}
	case 'D':
		return coord{X: p.X, Y: p.Y - s.Magnitude}
	default:
...
func parsePath(s string) []step {
	...
	for _, part := range strings.Split(s, ",") {
		var s step
		s.Direction = part[0]
		s.Magnitude, _ = strconv.Atoi(part[1:])
