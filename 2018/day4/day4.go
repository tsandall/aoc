...
func part1() int {
	...
	sort.Slice(events, func(i, j int) bool {
		return events[i].Timestamp.LessThan(events[j].Timestamp)
	})

	guards := getGuardSummary(events)
	var max *guard

	for _, g := range guards {
		if max == nil || max.Total < g.Total {
			max = g
		}
	}

	sleepiest := maxValue(max.Asleep)

	return max.ID * sleepiest
}
...
func part2() int {
	...
	guards := getGuardSummary(events)
	var mostFrequent *guard

	for _, g := range guards {
		if mostFrequent == nil || g.Asleep[maxValue(g.Asleep)] > mostFrequent.Asleep[maxValue(mostFrequent.Asleep)] {
			mostFrequent = g
		}
	}

	return mostFrequent.ID * maxValue(mostFrequent.Asleep)
}

func maxValue(vs []int) int {
	pos := -1
	for i := range vs {
		if pos == -1 || vs[i] >= vs[pos] {
			pos = i
		}
	}
	return pos
...
type guard struct {
	ID           int   // guard ID
	Total        int   // aggregate of time spent sleeping
	Asleep       []int // sleep frequency
	fellAsleepAt int   // minute started sleeping (used to build summary)
}

func getGuardSummary(events []event) map[int]*guard {
	...
	for _, event := range events {
		switch event.Kind {
		case startShift:
			g, ok := guards[event.GuardID]
			if !ok {
				g = &guard{ID: event.GuardID, Asleep: make([]int, 60)}
				guards[event.GuardID] = g
			}
			curr = g
		case fallAsleep:
			curr.fellAsleepAt = event.Timestamp.Minute
		case wakeUp:
			for i := curr.fellAsleepAt; i < event.Timestamp.Minute; i++ {
				curr.Asleep[i]++
				curr.Total++
			}
		}
...
func parseInput(input string) []event {
	var result []event
	for _, line := range strings.Split(strings.TrimSpace(input), "\n") {
		parts := splitLine(line)
		id, kind := parseEventData(parts[6])
...
var lineRegexp = regexp.MustCompile(`\[(\d\d\d\d)-(\d\d)-(\d\d) (\d\d):(\d\d)\] (.+)`)

func parseEventData(text string) (int, eventKind) {
	...
	matches := shiftRegexp.FindStringSubmatch(text)
	if len(matches) != 2 {
		panic("bad event data")
	}

	return atoi(matches[1]), startShift
}

var shiftRegexp = regexp.MustCompile(`Guard #(\d+) begins shift`)

const wakeUp = "wakeUp"
const fallAsleep = "fallAsleep"
...
func (ts timestamp) slice() []int {
	return []int{ts.Year, ts.Month, ts.Day, ts.Hour, ts.Minute}
}
